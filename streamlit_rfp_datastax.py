...
ASTRA_DB_TABLE_NAME = 'fiddler_rfp_snippets_openai'
...
template = """You are a tool called RFP Helper. 
...
def store_query(
    ...
    astraSession.execute(
                "INSERT INTO fiddlerai.fiddler_rfp_history \
...
def store_feedback(uuid, feedback=-1):
    ...
    astraSession = st.session_state[DB_CONN]
    astraSession.execute(
                f"UPDATE fiddlerai.fiddler_rfp_history SET feedback = {feedback} WHERE row_id = '{uuid}'"
    )
    return
...
def store_comment(uuid):
    ...
    astraSession = st.session_state[DB_CONN]
    astraSession.execute(
                f"UPDATE fiddlerai.fiddler_rfp_history SET comment = '{comment}' WHERE row_id = '{uuid}'"
    )
    st.session_state[COMMENT] = ""
    return
...
def main():
    ...
    if prompt := st.chat_input("Ask your questions about Fiddler here."):
    ...
    if st.session_state[ANSWER] is not None:
        ...
        with st.expander("Click here to leave your feedback on the helper's response"):
